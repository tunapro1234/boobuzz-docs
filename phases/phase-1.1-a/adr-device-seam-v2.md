...
| `hood_left` / `hood_right` | typed declarations use left `FORWARD` (amended 25 Sep, B06: `REVERSE` double-inverted the `1-u` command; the only left inversion is the `1-u` position mapping in the profile) and right `FORWARD`; `initialPos` `1.0`/`0.0` is the simulator hold value only — the real robot writes no hood position at init, matching the archive; archive `HoodSubsystem` has no separate `Servo.setDirection`, so preserve `rightInverse=false` complementary logic. The 25° stow vector is `(1.0, 0.0)`; 44° default is `(.4553333333, .5446666667)` |
